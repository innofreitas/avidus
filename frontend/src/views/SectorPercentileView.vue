...
const rows      = ref<SectorPercentileRow[]>([]);
const loading   = ref(false);
const error     = ref<string | null>(null);
const filter    = ref("");
const sortField = ref<"ticker" | "date" | "composite" | "updatedAt">("ticker");
const sortDir   = ref<"asc" | "desc">("asc");

// Um único id expandido — ref<string|null> é 100% reativo no Vue 3
const expandedId = ref<string | null>(null);
const activeTab  = ref("percentiles");
...
function toggleExpand(row: SectorPercentileRow) {
  if (expandedId.value === row.id) {
    expandedId.value = null;
  } else {
    expandedId.value = row.id;
    activeTab.value  = "percentiles";
  }
}
...
const TABS = [
  { key: "percentiles", label: "Percentis (11)"      },
  { key: "factors",     label: "Fatores (4)"         },
...
const tabContent = computed(() => {
  ...
  if (activeTab.value === "percentiles") return row.percentiles;
  if (activeTab.value === "factors") return row.factors;
  if (activeTab.value === "composite") return { composite: row.composite };
...
async function deleteEntry(row: SectorPercentileRow) {
  const { isConfirmed } = await MySwal.fire({
    title: "Remover percentis",
    html: `Deseja remover os percentis setoriais de <strong class="font-mono text-indigo-600 dark:text-indigo-400">${row.ticker}</strong>?`,
    icon: "warning",
  ...
  deleting.value = true;
  try {
    await api.delete(`/stock/sector-percentiles/${row.ticker}`);
    if (expandedId.value === row.id) expandedId.value = null;
    rows.value = rows.value.filter(r => r.id !== row.id);
  } catch (e: any) {
    await MySwal.fire({
      title: "Erro ao remover",
      text: e?.message ?? "Não foi possível remover os percentis.",
      icon: "error",
...
<template>
  <div class="max-w-7xl mx-auto space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-lg font-bold">📊 Percentis Setoriais</h1>
        <p class="text-xs text-gray-500 mt-0.5">
          Comparação de ativos com peers setoriais —
    ...
    <div v-else-if="!loading && rows.length === 0"
      ...
      <p class="text-xs mt-1">Abra a comparação setorial no portfólio para gerar dados</p>
    </div>
      ...
      <table class="w-full text-sm">
        ...
        <thead class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <tr class="text-left">
            ...
            <th class="py-3 px-4 font-semibold text-gray-600 dark:text-gray-300">
              Setor
            </th>
            ...
            <th class="py-3 px-4">
              <button @click="setSort('composite')"
                class="flex items-center gap-1.5 font-semibold text-gray-600 dark:text-gray-300
                       hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                Score <span class="text-gray-400 text-xs">{{ sortIcon("composite") }}</span>
              </button>
            </th>
        ...
        <tbody>
          <template v-for="row in paginated" :key="row.id">
            ...
            >
              ...
              <!-- Score Composto -->
              <td class="py-3 px-4">
                <span class="font-semibold font-mono text-sm" :style="{ color: getScoreColor(row.composite) }">
              ...
              <td class="py-3 px-4">
                <div class="flex items-center justify-center gap-2">
                  ...
                  >
                    {{ expandedId === row.id ? "▲ Ocultar" : "▼ Dados" }}
                  </button>
            ...
            <tr v-if="expandedId === row.id" :key="row.id + '_exp'">
              <td colspan="6" class="p-0 border-b border-indigo-200 dark:border-indigo-800">
