...
import { formatPercent, formatNumber, decisionColor, decisionBadgeClass } from "@/shared/utils/formatters";
...
const activeScore  = computed(() => result.value?.scores?.[activeProfile.value] ?? null);
const activeColor  = computed(() => activeScore.value ? decisionColor(activeScore.value.decision) : "#6b7280");

// Analisa um ticker — limpa resultado anterior antes de buscar
...
async function refresh() {
  ...
  try {
    await store.invalidateCache(result.value.meta.ticker);
    await store.analyze(result.value.meta.ticker);
  } finally {
  ...
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="card space-y-4">
      <div>
        ...
        <p class="text-xs text-gray-500 mt-0.5">Selecione o perfil de investidor e digite o ticker</p>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <button v-for="p in ALL_PROFILES" :key="p"
          ...
          <span class="text-xl mb-1">{{ PROFILE_ICONS[p] }}</span>
          <span :class="['text-sm font-semibold', activeProfile === p ? 'text-indigo-700 dark:text-indigo-300' : '']">
            {{ PROFILE_LABELS[p] }}
          </span>
          <span class="text-xs text-gray-400 leading-tight mt-0.5">{{ PROFILE_DESCS[p] }}</span>
        </button>
      </div>
    ...
    <!-- ── Resultados ──────────────────────────────────────── -->
    <template v-if="result && !store.loading">
      ...
      <div class="card">
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <div class="text-right">
            ...
            <button @click="refresh" :disabled="refreshing" class="btn-secondary text-xs mt-2">
      ...
      <!-- Grid de todos os perfis (clicável) -->
      <div>
      ...
      <!-- Tabela de indicadores do perfil ativo -->
    ...
    <!-- Empty state -->
    <div v-if="!result && !store.loading && !store.error" class="text-center py-20 text-gray-400">
      <p class="text-6xl mb-4">📊</p>
      <p class="text-xl font-semibold text-gray-600 dark:text-gray-300">Pronto para analisar</p>
      <p class="text-sm mt-2">Selecione um perfil, digite o ticker e clique em Analisar</p>
    </div>
